...
if(isset($_POST['submit'])){
    ...
    $SubTotalAmount = addslashes(trim($_POST['SubTotalAmount']));
    $CreatedDate = date('Y-m-d');
    $sql = "INSERT INTO tbl_transfer_godown_raw_prod_stock_2025 SET GodownId='$GodownId',FranchiseId='$FranchiseId',StockDate='$StockDate',TotQty='$TotQty',Narration='$Narration',CreatedBy='$user_id',CreatedDate='$CreatedDate',TotalAmount='$TotalAmount',InvoiceNo='$InvoiceNo',GstAmount='$GstAmount',OwnShop=1,SubTotalAmount='$SubTotalAmount',DiscPer='$DiscPer',Discount='$Discount'";
    $conn->query($sql);
...
<label class="form-label"> To Franchise <span class="text-danger">*</span></label>
 <select class="select2-demo form-control" name="ToFrId" id="ToFrId" required onchange="getFrTransferProduct(this.value)">

<option selected="" value="">Select Franchise</option>
<?php 
  $sql12 = "SELECT * FROM tbl_users_bill WHERE Roll=5 AND Status=1 AND id!='$BillSoftFrId' ORDER BY ShopName";
  $row12 = getList($sql12);
  foreach($row12 as $result){
     ?>
...
<option selected="" value="">Select Product</option>
 <?php 
  $sql12 = "SELECT id,ProductName,ProdType2 FROM tbl_cust_products_2025 WHERE Status='1' AND CreatedBy='$BillSoftFrId' AND ProdType=0 AND ProdType2 IN (1,3) AND checkstatus=1  ORDER BY ProductName";
  $row12 = getList($sql12);
  foreach($row12 as $result){
...
<div class="clearfix"></div>
     ...
     <div class="form-group col-md-2">
<label class="form-label">Sub Total Amount <span class="text-danger">*</span></label>
<input type="text" name="SubTotalAmount" id="SubTotalAmount" class="form-control" placeholder="" value="<?php echo $row7["SubTotalAmount"]; ?>" autocomplete="off" readonly>
<div class="clearfix"></div>
    </div>
    
     <div class="form-group col-md-2">
<label class="form-label">Discount % <span class="text-danger">*</span></label>
<input type="text" name="DiscPer" id="DiscPer" class="form-control" placeholder="" value="<?php echo $row7["DiscPer"]; ?>" autocomplete="off" required oninput="calDiscount()">
<div class="clearfix"></div>
    </div>
...
<?php include_once 'footer_script.php'; ?>
 ...
 function calDiscount() {
    ...
    var SubTotalAmount = parseFloat($('#SubTotalAmount').val()) || 0;

    // Calculate discount
    var Discount = (SubTotalAmount * DiscPer) / 100;
      ...
      var action = "calTotalQty";
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#TotQty').val(res.Qty);
                    $('#SubTotalAmount').val(parseFloat(res.TotalPrice).toFixed(2));
