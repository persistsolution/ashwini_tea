...
if ($_POST['action'] == 'matchMrpProduct') {
    ...
    $FrId = $_POST['FrId'];
    $sql = "SELECT ProdId FROM tbl_cust_products_2025 WHERE id='$godown_prodid'";
$row = getRecord($sql);
$ProdId = $row['ProdId'];
$sql = "SELECT id FROM tbl_cust_products_2025 WHERE ProdId='$ProdId' AND CreatedBy = '$godown_prodid'";
$row = getRecord($sql);?>
      <option selected value="0" style="font-weight:bold;color:#666;">-- Select Product --</option>
      <?php 
    ...
    foreach ($row12 as $result) {
        ...
        ?>
        <option value="<?php echo $result['id']; ?>" 
                style="color: <?php echo $color; ?>; font-weight:bold; padding:4px;" <?php if($result['ProdId'] == $row['id']){?>selected<?php } ?>>
            <?php echo htmlspecialchars($result['ProductName']) . 
                   " / ₹" . number_format($result['MinPrice'], 2) . 
                   " / " . $ProdTypeName; ?>
        </option>
