...
<?php 
$q = "SELECT * FROM tbl_cust_category_2025 WHERE Status='1' AND ProdType=1 ORDER BY id DESC";
$r = $conn->query($q);
while($rw = $r->fetch_assoc()) {
    // Check OldNew value
    $label = $rw['Name'] . ' (' . ($rw['OldNew'] == 1 ? 'New' : 'Old') . ')';
?>
...
<label class="form-label">Catgeory Type <span class="text-danger">*</span></label>
...
function update_toast(){
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      $('#OldNew').attr("selected","selected").val(1);
       $('#submit').text('Submit');
 ...
 $.ajax({  
                ...
                success:function(data){  
                 ...
                 $('#OldNew').val(res.OldNew).attr("selected",true);  
                     $('#action').val('Edit'); 
