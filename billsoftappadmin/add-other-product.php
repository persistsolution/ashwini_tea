...
<input type="text" id="CgstPer" name="CgstPer" class="form-control" value="2.5" required="" onKeyPress="return isNumberKey(event)" oninput="getProdPrice(document.getElementById('CgstPer').value,document.getElementById('SgstPer').value,document.getElementById('IgstPer').value,document.getElementById('DiscPer').value,document.getElementById('SubTotal').value)">
...
function getProdPrice(CgstPer, SgstPer, IgstPer, DiscPer, SubTotal) {
    ...
    // Step 2: Final Price after discount (still inclusive of GST)
    var FinalPrice = Number(SubTotal) - Number(DiscAmt);
    $('#MinPrice').val(parseFloat(FinalPrice).toFixed(2));

    // Step 3: Total GST %
    var GstPer = Number(CgstPer) + Number(SgstPer) + Number(IgstPer);

    // Step 4: Calculate Base Price (excluding GST)
    var MinPrice = Number(FinalPrice) / (1 + (GstPer / 100));
    ...
    // Step 6: Split into CGST, SGST, IGST
    var CgstAmt = Number(MinPrice) * (Number(CgstPer) / 100);
    var SgstAmt = Number(MinPrice) * (Number(SgstPer) / 100);
    var IgstAmt = Number(MinPrice) * (Number(IgstPer) / 100);
