...
$query = "SELECT * FROM tbl_users_bill WHERE Phone = '$Username' AND Status=1 AND Roll IN (93)";
 $rncnt = getRow($query);
 $row = getRecord($query);
 if($rncnt > 0){
...
if($YourOtp == $GetOtp){
$query = "SELECT * FROM tbl_users_bill WHERE Phone = '$Phone' AND Status=1 AND Roll IN (93)";
$rncnt = getRow($query);
if($rncnt > 0){
$row = getRecord($query);
$_SESSION['User'] = $row;
