...
<?php 
  $sql12 = "SELECT id,ProductName,CatId FROM tbl_cust_products_2025 WHERE Status='1' AND CreatedBy='$FranchiseId' AND ProdType=0 AND ProdType2 IN (1) AND delete_flag=0 AND checkstatus=1 ORDER BY ProductName";
  $row12 = getList($sql12);
  foreach($row12 as $result){
      $sql3 = "SELECT Name FROM tbl_cust_category_2025 WHERE id='".$result['CatId']."'";
      $row3 = getRecord($sql3);
     ?>
  <option <?php if($row7["ProdId"] == $result['id']) {?> selected <?php } ?> value="<?php echo $result['id'];?>">
    <?php echo $result['ProductName']." (".$row3['Name'].")"; ?></option>
<?php } ?>
</select>
