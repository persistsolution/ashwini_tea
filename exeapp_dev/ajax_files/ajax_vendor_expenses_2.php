...
if (isset($_FILES['Photo2']['name']) && $_FILES['Photo2']['name'] != '') {
    ...
    // Use pathinfo for safer extension handling
    ...
    if ($ext2 == 'pdf') {
        ...
        // Remove spaces, keep filename without extension
        $fnm = pathinfo($FileName2, PATHINFO_FILENAME);
        $fnm = str_replace(" ", "_", $fnm);

        // Always force .pdf extension
        $dest = '../../uploads/' . $randno . "_" . $fnm . ".pdf";
        $imagepath = $randno . "_" . $fnm . ".pdf";

        if (move_uploaded_file($src, $dest)) {
            $Photo2 = $imagepath;
        } else {
            $Photo2 = $_POST['OldPhoto2']; // fallback if upload fails
        }
    } else {
